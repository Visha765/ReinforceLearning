...
\matref{Q1}、\matref{Q2}に基づいて、テーブルqTableの更新、則ち学習を行う。
...
\srcref{mainQTable}に示すmainQTable.pyでは、テーブルQ学習を行う実行スクリプトとして実装した。
\par
% まず、5-23行目に示すコンストラクタでは、
% 状態空間と行動空間の各次元の分割数K、Lを引数に取り、Qテーブルとして
% \secref{sec:qTable}で実装したQTableクラスのインスタンスqTableを作成している。
...
\lstinputlisting[caption = mainQTable.py ,label = mainQTable]{exec/mainQTable.py}
